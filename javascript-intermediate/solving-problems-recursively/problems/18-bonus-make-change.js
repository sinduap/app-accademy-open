...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  for (let i = 0; i < coins.length; i++) {
    const coin = coins.at(i);
    const diff = target - coin;
    if (diff >= 0) {
      const restChanges = greedyMakeChange(diff, coins);
      if (!restChanges) return changes;
      changes = [...restChanges, coin];
      break;
    }
  }
  return changes;
...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  ...
  for (let i = 0; i < coins.length; i++) {
    ...
    if (diff >= 0) {
      const restChanges = greedyMakeChange(diff, coins);
      if (!restChanges) return changes;
      possibleChanges.push([...restChanges, coin]);
    }
  }
