...
function countConsonants(word) {
  ...
  const cleanedWord = word.split(' ').join('');
  let count = 0;
...
function alternatingLetters(str) {
  let isCurrentUpperCase = false;
  let result = '';

  for (let i = 0; i < str.length; i++) {
    const char = str[i];
    result += isCurrentUpperCase ? char.toUpperCase() : char.toLowerCase();
    isCurrentUpperCase = !isCurrentUpperCase;
  }
